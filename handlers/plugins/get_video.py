...
async def cmd(event):
    ...
    if link_soup:
        ...
        async def callback(current, total):
            ...
            if 20.0 < percent < 30.0 :
                if message.text == 'Downloading ...': 
                    await client.edit_message(entity, msg.id, text=f"'Uploaded 25 out of', {convert(total)}")
            
            elif  50.0 < percent < 60.0 :
                if message.text == f"'Uploaded 25 out of', {convert(total)}": 
                    await client.edit_message(entity, msg.id, text=f"'Uploaded 50 out of', {convert(total)}")
            
            elif 70.0 < percent < 80.0 :
                if message.text == f"'Uploaded 50 out of', {convert(total)}": 
                    await client.edit_message(entity, msg.id, text=f"'Uploaded 75 out of', {convert(total)}")
            
            elif 80.0 < percent < 95.0 :
                if message.text == f"'Uploaded 75 out of', {convert(total)}": 
                    await client.edit_message(entity, msg.id, text=f"'Uploaded 95 out of', {convert(total)}")
        ...
        ff = FFmpeg(inputs={video: None}, outputs={"output.png": ['-ss', '00:00:02', '-vframes', '1']})
        ff.run()
    
        await client.send_file(entity=entity, file= video, progress_callback=callback, attributes=(DocumentAttributeVideo(int(float(duration)),width,height,supports_streaming=True),), thumb='output.png')
